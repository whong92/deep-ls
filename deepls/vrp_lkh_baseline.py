...
from deepls.generate_vrp_data import check_extension, save_dataset, load_dataset
import argparse
import numpy as np
import re
...
if __name__ == "__main__":
    ...
    dataset = load_dataset(dataset_path)[:num_data]
    ...
    N = len(dataset)
    instance_count = 0
    for batch in tqdm(range(int(np.ceil(N / num_workers)))):
        start = batch * num_workers
        end = min((batch + 1) * num_workers, N)
        batch_instances = dataset[start:end]
        args = []
        for instance in batch_instances:
            depot, loc, demand, capacity = instance
            args.append((
                executable, workdir, f'instance-{instance_count:06d}',
                depot, loc, demand, capacity, 1
            ))
            instance_count += 1
        batch_sol = pool.starmap(
            solve_lkh_log, args
        )

        for sol, instance in zip(batch_sol, batch_instances):
            depot, loc, demand, capacity = instance
            results.append({
                'depot': depot,
                'nodes': loc,
    ...
    #     depot, loc, demand, capacity = instance
    #     result = solve_lkh_log(
    #         executable,
    #         workdir, f'instance-{i:06d}',
    #         depot, loc, demand, capacity, 1
    #     )
    #     results.append({
    #         'depot': depot,
    ...
    costs = [r['opt_dist'] for r in results]
