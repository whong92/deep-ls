...
static void PyDict_SetItemStringAndDecRef(PyObject* pydict, const char* str, PyObject* PyBuiltVal) {
    PyDict_SetItemString(pydict, str, PyBuiltVal);
    Py_DECREF(PyBuiltVal);
}

static PyObject* PyDict_GetItemAndDecRef(PyObject* pydict, PyObject* PyBuiltKey) {
    PyObject* ret = PyDict_GetItem(pydict, PyBuiltKey);
    Py_DECREF(PyBuiltKey);
    return ret;
}
...
bool check_relocate_move_valid(
    ...
    double dst_demand = PyFloat_AsDouble(PyDict_GetItemAndDecRef(cum_demand_dst,  Py_BuildValue("i", -1)));
...
PyObject* map_to_pydict(unordered_map<string, string> map) {
    ...
    for (it = map.begin(); it != map.end(); it++)
    {
        string k = it->first;
        string v = it->second;
        PyDict_SetItemStringAndDecRef(py_dict, k.c_str(), Py_BuildValue("s", v.c_str()));
    }
    return py_dict;
}
...
//                    ] = to_string(cost);
                    ...
                    PyDict_SetItemStringAndDecRef(nbh_dict, "nb_type", Py_BuildValue("s", "reloc"));
                    PyDict_SetItemString(nbh_dict, "tour0", tour0);
                    if(tour1 != Py_None) { PyDict_SetItemString(nbh_dict, "tour1", tour1); }
                    else { PyDict_SetItemStringAndDecRef(nbh_dict, "tour1", Py_BuildValue("i", -1)); }
                    PyDict_SetItemStringAndDecRef(nbh_dict, "src_node", Py_BuildValue("i", src_node));
                    PyDict_SetItemStringAndDecRef(nbh_dict, "src_u", to_py_tuple(src_u));
                    PyDict_SetItemStringAndDecRef(nbh_dict, "src_v", to_py_tuple(src_v));
                    PyDict_SetItemStringAndDecRef(nbh_dict, "dst_w", to_py_tuple(dst_w));
                    PyDict_SetItemStringAndDecRef(nbh_dict, "dst_wp", Py_BuildValue("(i,i)", get<0>(src_u), get<1>(src_v)));
                    PyDict_SetItemStringAndDecRef(nbh_dict, "src_up", Py_BuildValue("(i,i)", get<0>(dst_w), src_node));
                    PyDict_SetItemStringAndDecRef(nbh_dict, "src_vp", Py_BuildValue("(i,i)", src_node, get<1>(dst_w)));
                    PyDict_SetItemStringAndDecRef(nbh_dict, "cost", Py_BuildValue("d", cost));
                    PyDict_SetItemStringAndDecRef(
                        reloc_nbhs_dict,
...
bool check_cross_move_valid(
    ...
    double cum_dem_u = PyFloat_AsDouble(PyDict_GetItemAndDecRef(cum_demand_0,  Py_BuildValue("i", u)));
    double cum_dem_v = PyFloat_AsDouble(PyDict_GetItemAndDecRef(cum_demand_0,  Py_BuildValue("i", v)));
    double cum_dem_x = PyFloat_AsDouble(PyDict_GetItemAndDecRef(cum_demand_1,  Py_BuildValue("i", x)));
    double cum_dem_y = PyFloat_AsDouble(PyDict_GetItemAndDecRef(cum_demand_1,  Py_BuildValue("i", y)));

    double C0 = PyFloat_AsDouble(PyDict_GetItemAndDecRef(cum_demand_0,  Py_BuildValue("i", -1)));
    double C1 = PyFloat_AsDouble(PyDict_GetItemAndDecRef(cum_demand_1,  Py_BuildValue("i", -1)));
    ...
    for (int i=0; i<size; i++) {
        ...
        for (int j=0; j<n_pairs; j++) {
            ...
            if(!noop) {
                ...
                if(valid) {
                    PyObject* nbh_dict = PyDict_New();
                    PyDict_SetItemStringAndDecRef(nbh_dict, "nb_type", Py_BuildValue("s", "cross"));
                    PyDict_SetItemString(nbh_dict, "tour0", tour0);
                    PyDict_SetItemString(nbh_dict, "tour1", tour1);
                    PyDict_SetItemStringAndDecRef(nbh_dict, "e0", to_py_tuple(e0));
                    PyDict_SetItemStringAndDecRef(nbh_dict, "e1", to_py_tuple(e1));
                    PyDict_SetItemStringAndDecRef(nbh_dict, "e0p", to_py_tuple(e0p0));
                    PyDict_SetItemStringAndDecRef(nbh_dict, "e1p", to_py_tuple(e1p0));
                    PyDict_SetItemStringAndDecRef(nbh_dict, "cost", Py_BuildValue("d", cost0));
                    PyDict_SetItemStringAndDecRef(
                        cross_nbhs_dict,
            ...
            if(!noop) {
                ...
                if(valid) {
                    ...
                    PyDict_SetItemStringAndDecRef(nbh_dict, "e0", to_py_tuple(e0));
                    PyDict_SetItemStringAndDecRef(nbh_dict, "e1", to_py_tuple(e1));
                    PyDict_SetItemStringAndDecRef(nbh_dict, "e0p", to_py_tuple(e0p1));
                    PyDict_SetItemStringAndDecRef(nbh_dict, "e1p", to_py_tuple(e1p1));
                    PyDict_SetItemStringAndDecRef(nbh_dict, "cost", Py_BuildValue("d", cost1));
                    PyDict_SetItemStringAndDecRef(
                        cross_nbhs_dict,
...
//                twoopt_nbhs_dict[get_nbh_rep_key(normalized_nbh.edges_added, normalized_nbh.edges_removed)] = to_string(cost);
                ...
                PyObject* nbh_dict = PyDict_New();
                PyDict_SetItemStringAndDecRef(nbh_dict, "nb_type", Py_BuildValue("s", "2opt"));
                PyDict_SetItemString(nbh_dict, "tour_idx", tour0);
                PyDict_SetItemStringAndDecRef(nbh_dict, "e0", to_py_tuple(e0));
                PyDict_SetItemStringAndDecRef(nbh_dict, "e1", to_py_tuple(e1));
                PyDict_SetItemStringAndDecRef(nbh_dict, "e0p", to_py_tuple(e0p));
                PyDict_SetItemStringAndDecRef(nbh_dict, "e1p", to_py_tuple(e1p));
                PyDict_SetItemStringAndDecRef(nbh_dict, "cost", Py_BuildValue("d", cost));
                PyDict_SetItemStringAndDecRef(
                    twoopt_nbhs_dict,
